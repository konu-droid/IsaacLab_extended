...
class LeatherbackEnvCfg(DirectRLEnvCfg):
    ...
    camera_cfg = CameraCfg(
        prim_path="/World/envs/env_.*/Robot/leatherback/front_cam",
        update_period=0.1,
        height=480,
        width=640,
...
class LeatherbackEnv(DirectRLEnv):
    ...
    def _get_observations(self) -> dict:
        ...
        # very improtant since if the depth as inf values for nan the robot in simulation gliches and flys. 
        # sim does not crash or the network doesnt show any error.
        rgb_img = torch.clamp(rgb_img, min=0.0, max=255.0)
        depth_img = torch.clamp(depth_img, min=0.0, max=255.0)
        rgb_img = torch.div(rgb_img, 255.0)
        depth_img = torch.div(depth_img, 255.0)
    ...
    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        body_pos = self.car.data.root_pos_w
